...
- 2026-04-02：用户进一步确认项目汇报显示规则升级为“仪表盘版展示 + 分段流式发送”，并要求同时视为项目规则和全局默认规则。具体要求：减少整句加粗，更多使用轻标题、状态标签、进度条样式和留白；默认采用区块化面板样式展示“已实现能力 / 当前实现中 / 总盘子 / 阶段位置 / 评分 / 推荐方案 / 结论”；默认拆成 3～4 段发送，标准顺序为“状态 → 总盘子/评分 → 推荐方案 → 结论”。
