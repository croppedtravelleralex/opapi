...
- 2026-04-03：用户明确要求把“项目汇报模板 v1.2 + 仪表盘版展示 + 分段流式发送”正式设为**全局规则和项目规则**；后续不仅项目汇报使用该模板，也应视为默认全局输出规则之一。
- 2026-04-03：用户再次强调“不要光说不做”；当用户给出明确推进指令（尤其是只发 `1`）时，应直接执行，不要先做计划性空话或只做口头承诺。
