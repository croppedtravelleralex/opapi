...
- 用户新增默认执行规则：**收到任务先分析任务大小，再按大小决定拆分粒度，最后执行当前最值步骤**；同时默认要**减少 token 开销**，避免重排查、重总结、重复读写与大而全输出。
...
- 2026-04-03：用户明确重排 `openclaw-openai-gateway` 主线：**第三方 provider 不是核心，先跳过**；当前最核心功能应改为**从 Codex App / Web 会话中稳定反代出真实额度**，先扩充额度池，再谈对外提供 API Key、账号池、容灾、平衡与后续治理。
- 2026-04-03：用户明确要求项目汇报中的“大功能”要改成更外部、可理解的能力语言，例如“实现第三方 API Key 和 BaseURL 导入”，而不是只写内部模块名。
...
- 2026-04-03：用户新增全局默认推进规则：先分析任务大小，再按大小拆分，最后只执行当前最值步骤；并新增硬约束“默认优先减少 token 开销”。默认策略：小任务直接做、少废话；中任务简拆 3–4 步，先结论后执行；大任务先收边界再分阶段，必要时才进入重规划（如 planning-with-files / taskr）；同时默认少开工具、少读大文件、少做全量扫描、少重复总结、少讲长铺垫，优先做最便宜的确认动作。
